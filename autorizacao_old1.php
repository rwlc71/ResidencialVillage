...
// --- Cabeçalhos fixos com Cell --- //
    ...
    $pdf->Cell(55, 4, '', 0, 0); // espaço do valor

    $pdf->Cell(15, 4, utf8_decode('CPF/RG:'), 0, 0);
    $pdf->Cell(40, 4, '', 0, 0); // espaço do valor
...
// Variáveis formatadas
    ...
    $parentesco = strtoupper(trim($ln['parentesco']));

// --- Imprime os valores com MultiCell() nas posições corretas --- //
    $pdf->SetXY($xResp, $yLinha);
    $pdf->MultiCell(55, 4, utf8_decode($resp), 0, 'L');

    $pdf->SetXY($xDoc, $yLinha);
    $pdf->MultiCell(40, 4, utf8_decode($doc), 0, 'L');
...
// Ajuste Y para espaço de 2 linhas (12 pontos)
    $novaAltura = max($pdf->GetY(), $yLinha + 12);
    $pdf->SetY($novaAltura);
